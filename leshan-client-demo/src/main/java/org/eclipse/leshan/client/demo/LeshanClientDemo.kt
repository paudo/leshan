...
object LeshanClientDemo {
    ...
    fun createAndStartClient(endpoint: String, localAddress: String?, localPort: Int, needBootstrap: Boolean,
                             serverURI: String, pskIdentity: ByteArray?, pskKey: ByteArray?, clientPrivateKey: PrivateKey?, clientPublicKey: PublicKey?,
                             serverPublicKey: PublicKey?, clientCertificate: X509Certificate?, serverCertificate: X509Certificate?,
                             latitude: Float?, longitude: Float?, scaleFactor: Float) {

        locationInstance = MyLocation(latitude, longitude, scaleFactor)
        ...
        if (needBootstrap) {
            if (pskIdentity != null) {
                ...
                initializer.setInstancesForObject(SECURITY, rpkBootstrap(serverURI, clientPublicKey.encoded,
                        clientPrivateKey!!.encoded, serverPublicKey!!.encoded))
                initializer.setClassForObject(SERVER, Server::class.java)
            } else if (clientCertificate != null) {
                initializer.setInstancesForObject(SECURITY, x509Bootstrap(serverURI, clientCertificate.encoded,
            ...
            if (pskIdentity != null) {
                ...
                initializer.setInstancesForObject(SECURITY, rpk(serverURI, 123, clientPublicKey.encoded,
                        clientPrivateKey!!.encoded, serverPublicKey!!.encoded))
                initializer.setInstancesForObject(SERVER, Server(123, 30, BindingMode.U, false))
            } else if (clientCertificate != null) {
                initializer.setInstancesForObject(SECURITY, x509(serverURI, 123, clientCertificate.encoded,
                        clientPrivateKey!!.encoded, serverCertificate!!.encoded))
                initializer.setInstancesForObject(SERVER, Server(123, 30, BindingMode.U, false))
            } else {
        ...
        if (clientPublicKey != null) {
            if (clientPublicKey is ECPublicKey) {
                ...
                throw IllegalStateException("Unsupported Public Key Format (only ECPublicKey supported).")
            }
        }
